...
// Reusable CheckmarkIcon component
...
// Reusable CloseIconSvg component
...
/**
 * A general-purpose success popup modal.
 ...
 */
const SuccessPopup = ({ show, onClose, title, message }) => {
  ...
  const handleClose = () => {
    setIsExiting(true);
    setTimeout(onClose, 200); // Duration should match the exit animation
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[1001] p-4">
      ...
      >
        <div className="p-6 text-center">
            ...
            <button onClick={handleClose} className="w-full h-11 px-6 bg-blue-600 hover:bg-blue-700 rounded-md text-white font-sans font-semibold text-sm transition-colors">
                OK
            </button>
        </div>
      </div>
